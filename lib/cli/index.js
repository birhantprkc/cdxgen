...
import { parseBunLock } from "../helpers/bunutils.js";
...
import { buildDynamicComponents } from "../helpers/dynamic.js";
...
} from "../helpers/remote/huggingface.js";
import {
...
function getPackagePropertyValues(pkg, propertyName) {
...
function splitPackagePropertyList(value) {
  return String(value || "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
...
function getPackageBinCommandNames(pkg) {
...
function hasNpmBinCommandEvidence(allImports, pkg) {
...
export async function createJavaBom(path, options) {
  ...
  if (
    ...
    const wrapperPropPath = join(
      gradleRootPath,
      "gradle",
      "wrapper",
      "gradle-wrapper.properties",
    );
    if (safeExistsSync(wrapperPropPath)) {
      try {
        const propContent = readFileSync(wrapperPropPath, "utf-8");
        const distUrlLine = propContent
          .split("\n")
          .find((l) => l.startsWith("distributionUrl="));
        if (distUrlLine) {
          const distributionUrl = distUrlLine
            .split("=")[1]
            .trim()
            .replace(/\\:/g, ":");
          ...
          const gradleVersion = versionMatch ? versionMatch[1] : "unknown";
          thoughtLog(
          ...
          if (!toolsArr.find((c) => c.name === "gradle")) {
            const distShaLine = propContent
              .split("\n")
              .find((l) => l.startsWith("distributionSha256Sum="));
            let hashes;
            if (distShaLine) {
              const sha256Sum = distShaLine.split("=")[1].trim();
              if (sha256Sum) {
                hashes = [
                  {
                    alg: "SHA-256",
            ...
            const gradleComponent = {
              ...
              purl: `pkg:maven/org.gradle/gradle@${gradleVersion}?type=bin`,
              "bom-ref": `pkg:maven/org.gradle/gradle@${gradleVersion}?type=bin`,
              externalReferences: [
                {
                  type: "distribution",
                  url: distributionUrl,
    ...
    const gradleInitArgs = process.env.GRADLE_INCLUDED_BUILDS
      ? []
...
function getDirectAiInventoryType(path, options) {
  const exactAiInventoryType = getExactAiInventoryType(options);
  if (exactAiInventoryType) {
    ...
    for (const f of bunLockFiles) {
      ...
      if (safeExistsSync(packageJsonF)) {
        ...
        if (pcs.length && Object.keys(pcs[0]).length) {
          ...
          ppurl = new PackageURL(
            "npm",
            options.projectGroup || bunParentComponent.group,
            bunParentComponent.name,
            options.projectVersion || bunParentComponent.version,
            null,
            null,
          ).toString();
    ...
    for (const f of denoLockFiles) {
      ...
      if (denoJsonFile) {
        ...
        if (denoConfig?.name) {
          ...
          ppurl = new PackageURL(
            "npm",
            denoParentComponent.group,
            denoParentComponent.name,
            denoParentComponent.version,
            null,
            null,
          ).toString();
...
export async function createRubyBom(path, options) {
  ...
  const bundleWithout =
    bundleConfig.BUNDLE_WITHOUT || process.env.BUNDLE_WITHOUT;
