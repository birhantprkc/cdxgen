...
export function isSpdxLicenseExpression(license) {
  ...
  if (process.env.CDXGEN_LICENSE_ENHANCE !== "false") {
    ...
    return adjustLicenseInformation(
      license
        ...
        .map((l) => {
          ...
          const enableEnhance = process.env.CDXGEN_LICENSE_ENHANCE !== "false";
          const licenseRef = process.env.CDXGEN_LICENSE_REF === "true";
          if (enableEnhance) {
...
export async function getNpmMetadata(pkgList, registryUrl) {
  const NPM_URL =
    registryUrl || process.env.NPM_URL || "https://registry.npmjs.org/";
  const cdepList = [];
  for (const p of pkgList) {
    try {
      let key = p.name;
